...
class Collection extends React.Component {
    ...
    displayEvent= ()=>{
        let userObject = this.props.userList.filter(user=>user.userId===this.props.currentUser)[0]
        //console.log(userObject.collection.includes("2"))
        let eventArray = this.props.eventList.filter(item => userObject.collection.includes(item.eventId))
        //console.log(eventArray)
        let result = eventArray.map((item,index)=>{
            let user = this.props.userList.filter(user=>user.userId===item.userId)
            //console.log(user)
            return (<TouchableOpacity key = {index} style={styles.eventContainer} onPress = {()=>this.props.openEvent(item)}> 
